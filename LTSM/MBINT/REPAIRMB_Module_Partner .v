...
module REPAIRMB_Module_Partner (
    ...
    always @(posedge CLK or negedge rst_n) begin
        ...
        else if (i_RX_SbMessage == SB_MBINIT_REPAIRMB_END_REQ && i_msg_valid && 
    ...
    always @(*) begin
        ...
        case (CS)
            ...
            WAIT_START_REQ:
                if (!MBINIT_REVERSALMB_end)
                    NS = IDLE;
                else if (i_RX_SbMessage == SB_MBINIT_REPAIRMB_START_REQ && i_msg_valid)
            ...
            WAIT_FOR_REQUEST: begin
                if (!MBINIT_REVERSALMB_end)
                    NS = IDLE;
                else if ((i_RX_SbMessage == SB_MBINIT_REPAIRMB_END_REQ && i_msg_valid && continue_flag) ||
                         (end_req_received && continue_flag))  // ** Check stored flag**
                    NS = WAIT_SIDEBAND_END;
                else if (i_RX_SbMessage == SB_MBINIT_REPAIRMB_APPLY_DEGRADE_REQ &&
        ...
        if (!rst_n) begin
            ...
            case (NS)
                SEND_START_RESP: begin
                    ...
                    o_TX_SbMessage                 <= SB_MBINIT_REPAIRMB_START_RESP;
                end
                ...
                SEND_DEGRADE_RESP: begin
                    o_tx_data_valid_repair_partner <= 1'b1;
                    o_TX_SbMessage                 <= SB_MBINIT_REPAIRMB_APPLY_DEGRADE_RESP;
                end

                SEND_END_RESP: begin
                    o_tx_data_valid_repair_partner <= 1'b1;
                    o_TX_SbMessage                 <= SB_MBINIT_REPAIRMB_END_RESP;
                    continue_flag                  <= 1'b0;
                end
