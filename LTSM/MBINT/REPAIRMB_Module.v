...
module REPAIRMB_Module (
    ...
    always @(*) begin
        ...
        case (CS)
            ...
            WAIT_FOR_RESPONSE: begin
                if (!MBINIT_REVERSALMB_end) NS = IDLE;
                else if ((i_RX_SbMessage == SB_MBINIT_REPAIRMB_START_RESP && i_msg_valid) || i_Start_Repeater)
                    NS = PERFORM_D2C_TEST;
                else if (i_RX_SbMessage == SB_MBINIT_REPAIRMB_APPLY_DEGRADE_RESP && i_msg_valid && !apply_repeater) begin
                    if (w_Functional_Lanes == 2'b11 || valid_done || Go_to_done)
                        NS = WAIT_SIDEBAND_END;
                    else
                        NS = PERFORM_D2C_TEST;
                end
                else if (i_RX_SbMessage == SB_MBINIT_REPAIRMB_END_RESP && i_msg_valid)
                    NS = REPAIR_COMPLETE;
            end
            ...
            ANALYZE_LANE_STATUS:
                 if (!MBINIT_REVERSALMB_end) NS = IDLE;
                else if (done_setup) NS =WAIT_SIDEBAND_READY;
    ...
    always @(posedge CLK or negedge rst_n) begin
        if (!rst_n) begin
           ...
           if (i_RX_SbMessage == SB_MBINIT_REPAIRMB_APPLY_DEGRADE_RESP && 
            ...
            case (NS)
                SEND_START_REQ: begin
                    ...
                    o_TX_SbMessage <= SB_MBINIT_REPAIRMB_START_REQ;
                end
                ...
                SEND_DEGRADE_REQ: begin
                    o_tx_data_valid_repair <= 1;
                    o_TX_SbMessage <= SB_MBINIT_REPAIRMB_APPLY_DEGRADE_REQ;
                    o_Functional_Lanes <= w_Functional_Lanes;
                ...
                SEND_END_REQ: begin
                    o_tx_data_valid_repair <= 1;
                    o_TX_SbMessage <= SB_MBINIT_REPAIRMB_END_REQ;
                    valid_done <= 0;
                    Go_to_done <= 0;
                end
