...
always @(*) begin
    ...
    WAIT_FOR_RESPONSE:
        if(i_Rx_SbMessage==SB_MBINIT_REPAIRVAL_INIT_RESP && i_msg_valid)
            next_state = RUN_VALIDATION_PATTERN;
        else if(i_Rx_SbMessage==SB_MBINIT_REPAIRVAL_RESULT_RESP && i_msg_valid)
            next_state = VERIFY_RESULT;
        else if(i_Rx_SbMessage==SB_MBINIT_REPAIRVAL_DONE_RESP && i_msg_valid)
...
always @(posedge CLK or negedge rst_n) begin
    if(!rst_n) begin
        ...
        SEND_INIT_REQUEST: begin
            ...
            o_TX_SbMessage         <= SB_MBINIT_REPAIRVAL_INIT_REQ;
        end
        ...
        SEND_RESULT_REQUEST: begin
            o_ValidOutDatat_Module <= 1;
            o_TX_SbMessage         <= SB_MBINIT_REPAIRVAL_RESULT_REQ;
        end
        ...
        SEND_DONE_REQUEST: begin
            o_ValidOutDatat_Module <= 1;
            o_TX_SbMessage         <= SB_MBINIT_REPAIRVAL_DONE_REQ;
        end
