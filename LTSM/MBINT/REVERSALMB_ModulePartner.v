...
////////////////////////////////////////////////////////////////////////////////
    always @(*) begin
        ...
        case (CS)
            ...
            WAIT_FOR_INIT_REQUEST: begin
                ...
                else if (i_Rx_SbMessage == SB_MBINIT_REVERSALMB_INIT_REQ && i_msg_valid) NS = WAIT_BUSY_CLEAR_FOR_INIT_RESP;
            end
            ...
            WAIT_FOR_REQUEST: begin
                ...
                else if (i_Rx_SbMessage == SB_MBINIT_REVERSALMB_CLEAR_ERROR_REQ && i_msg_valid && ~i_Second_Clear_Error_Req ) NS = WAIT_BUSY_CLEAR_FOR_ERROR_RESP;
                else if (i_Rx_SbMessage == SB_MBINIT_REVERSALMB_RESULT_REQ && i_msg_valid) NS = WAIT_BUSY_CLEAR_FOR_RESULT_RESP;
                else if (i_Rx_SbMessage == SB_MBINIT_REVERSALMB_DONE_REQ && i_msg_valid && ~apply_repeater) NS = WAIT_BUSY_CLEAR_FOR_DONE_RESP;
            end
            WAIT_FOR_SECONDE_REQ: begin
                if (~i_MBINIT_REPAIRVAL_end) NS = IDLE;
                else if (i_Rx_SbMessage == SB_MBINIT_REVERSALMB_CLEAR_ERROR_REQ && i_msg_valid) NS = WAIT_BUSY_CLEAR_FOR_ERROR_RESP;
            end
    ...
    always @(posedge CLK or negedge rst_n) begin
        if (!rst_n) begin
            ...
            o_TX_SbMessage                          <= SB_MBINIT_REVERSALMB_INIT_RESP;
            ...
            case (NS)
                ...
                SEND_CLEAR_ERROR_RESPONSE: begin
                    o_tx_data_valid_reversal   <= 1'b1;
                    o_TX_SbMessage             <= SB_MBINIT_REVERSALMB_CLEAR_ERROR_RESP;
                    o_Clear_Pattern_Comparator <= 2'b01;
                end
                SEND_RESULT_RESPONSE: begin
                    o_tx_data_valid_reversal                 <= 1'b1;
                    o_TX_SbMessage                           <= SB_MBINIT_REVERSALMB_RESULT_RESP;
                    o_ValidDataFieldParameters_modulePartner <= 1'b1;
                    o_REVERSAL_Pattern_Result_logged         <= i_REVERSAL_Result_logged;
                end
                SEND_DONE_RESPONSE: begin
                    o_tx_data_valid_reversal <= 1'b1;
                    o_TX_SbMessage           <= SB_MBINIT_REVERSALMB_DONE_RESP;
                end
                REVERSAL_PARTNER_COMPLETE: begin
                    o_ValidDataFieldParameters_modulePartner <= 0;
                    o_tx_data_valid_reversal                 <= 1'b0;
