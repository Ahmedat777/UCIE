module mbtrain_linkspeed_tx # ( parameter SB_MSG_WIDTH = 4, parameter LANE_STATUS_WIDTH = 2)
 (
    ...
    IDLE ,
    START ,
    CHECK_RX_DONE, // we need to be sure the rx is not sending becuse if both tx and rx are sending at the same time, the priority is for the rx messages.
    POINT_TEST ,
    ANALYSIS ,
    ERROR_HANDLING ,
    EXIT_PHY_RETRAIN ,
    ...
    always @(*) begin
        ...
        case(current_state_tx)
            ...
            IDLE : next_state_tx = (i_enable)? CHECK_RX_DONE : IDLE;
            ...
            EXIT_PHY_RETRAIN: begin
                if(!i_enable)
                    next_state_tx = IDLE;
                else if(i_sb_valid && i_sb_msg == EXIT_TO_PHYRETRAIN_RESP)
                    next_state_tx = DONE;
                else 
                    next_state_tx = EXIT_PHY_RETRAIN;
            end
            ...
            EXIT_SPEED_DEGRADE: begin
                if(!i_enable) 
                    next_state_tx = IDLE;
                else if(i_sb_valid && i_sb_msg == EXIT_TO_SPEED_DEGRADE_RESP) 
                    next_state_tx = DONE;
                else
                    next_state_tx = EXIT_SPEED_DEGRADE;
            end
            ...
            DONE: begin
                if(!i_enable) 
                    ...
                    next_state_tx = EXIT_PHY_RETRAIN;
                    /* partner forces ERROR handling */
                else if(i_sb_valid && i_sb_msg == ERROR_REQ) 
                    next_state_tx = ERROR_HANDLING;
                     /* normal done resp path */
        ...
        if(!i_rst_n) begin
            ...
            case(next_state_tx)
                ...
                DONE: begin
                    ...
                    else if(i_sb_valid && i_sb_msg == ERROR_REQ) begin
                        o_timeout_disable <= 1; 
                        o_sb_msg <= 0;
                    end
                    else
