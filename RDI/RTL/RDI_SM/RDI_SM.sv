...
module  RDI_SM #( parameter int NBYTES = 64,        // these parameters are like those in RDI_if and can be used for array sizing and ther purposes within the state machine as needed.o
    ...
    input logic stall_handshake_done,    
    output logic errortime_done,         
    RDI_if.lphy lphy    
);
/*
...
always_ff @(posedge lphy.clk or negedge lphy.rstn) begin
    $monitor ($time, " Link Error Counter: %d", linkerror_counter);
    if (!lphy.rstn)
    begin
...
always_comb begin
    ...
    next_state = (lphy.pl_state_sts); // default to current state;
    case(lphy.pl_state_sts) 
        STS_RDI_RESET: begin
            if(lphy.lp_linkerror) //his transition is permitted if requested by the remote Link partner through the relevant sideband message.
                next_state = STS_LINK_ERROR;
            else if(nop_to_active && active_handshake_done ) // && active hanshake done through sideband message
                next_state = STS_RDI_ACTIVE;
            else if (nop_to_linkreset && active_handshake_done ) // || when requested by remote Link partner through the relevant sideband message.
            begin
                next_state = STS_RDI_ACTIVE;
            /*if (active_handshake_done)
                next_state = STS_LINK_RESET;*/
            end
            else if (nop_to_disable && active_handshake_done ) // || when requested by remote Link partner through the relevant sideband message.
            begin
                next_state = STS_RDI_ACTIVE;
            /*if (active_handshake_done)
        ...
        STS_RDI_ACTIVE: begin
            if(lphy.lp_linkerror) 
                next_state = STS_LINK_ERROR;
            else if (stall_handshake_done)
                begin
                    if(lphy.lp_state_req == REQ_RDI_RETRAIN || retrain_req) // || internal request to retrain the Link 
                    next_state = STS_RDI_RETRAIN;    
                    else if(  (lphy.lp_state_req == REQ_LINK_RESET || sb_msg_id_rdi == SB_RDI_REQ_LINKRESET)) 
                    next_state = STS_LINK_RESET;    
                    else if(  (lphy.lp_state_req == REQ_RDI_DISABLED || sb_msg_id_rdi == SB_RDI_REQ_DISABLE)) 
                    next_state = STS_RDI_DISABLED;
                end
            end
        STS_RDI_RETRAIN: begin
            if(lphy.lp_linkerror) // || when the Link detects an error condition that requires a transition to this state.
                next_state = STS_LINK_ERROR;
            else if(nop_to_active || (lphy.lp_state_req == REQ_LINK_RESET) || (lphy.lp_state_req == REQ_RDI_DISABLED)) begin 
                if(active_handshake_done)
                next_state = STS_RDI_ACTIVE;
            end
            end
        STS_LINK_RESET: begin
            if(lphy.lp_linkerror) // || when the Link detects an error condition that requires a transition to this state.
                next_state = STS_LINK_ERROR;
            else if (!lphy.rstn || lphy.lp_state_req == REQ_RDI_ACTIVE)
                next_state = STS_RDI_RESET;
            else if (lphy.lp_state_req == REQ_RDI_DISABLED)
                next_state = STS_RDI_DISABLED;
            end
        ...
        STS_LINK_ERROR: begin
            if (!lphy.rstn || ((lphy.lp_state_req == REQ_RDI_ACTIVE) && !lphy.lp_linkerror && errortime_done && !internal_error_hold))
                next_state = STS_RDI_RESET;
        end
